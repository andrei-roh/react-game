...
export class Game extends React.Component {
  state = initialData;
  shuffle = this.state.columns.column1.letterIds.sort(
    () => Math.round(Math.random() * 100) - 50
  );

  dataState = gameData;
  changeData =
    this.dataState.list[Math.floor(Math.random() * this.dataState.list.length)];
  showVictory = false;
  ...
  onDragEnd = (result) => {
    ...
    let gameResult = newFinish.letterIds.map(
      (letterId) => this.state.letters[letterId].content
    );
    if (gameResult.join('') === this.dataState.data[this.changeData].answer) {
      ...
      this.changeData =
        this.dataState.list[
          Math.floor(Math.random() * this.dataState.list.length)
        ];
    ...
    return (
      <GameBlock>
        <ImageBlock>
          <img
            width="200px"
            src={this.dataState.data[this.changeData].source}
            alt={this.dataState.data[this.changeData].title}
          />
        </ImageBlock>
        <DragDropContext
        ...
        >
          <Container>
            {this.state.columnOrder.map((columnId) => {
              ...
              const letters = column.letterIds.map(
                (letterId) => this.state.letters[letterId]
              );
              return (
                <Column key={column.id} column={column} letters={letters} />
              );
            })}
          </Container>
        </DragDropContext>
